package com.mygdx.game.mario.objects.mario;
...
import com.mygdx.game.mario.objects.particles.SmokeParticle;
import com.mygdx.game.mario.objects.particles.SparkleParticle;
...
public class Mario extends CollidableObject{
	...
	TextureRegion[] l_Big, r_Big;
    TextureRegion[] l_small, r_small;
    TextureRegion[] l_Fire, r_Fire;
    TextureRegion[] swim_left_small, swim_left_big,swim_left_fire;
    TextureRegion[] swim_right_small, swim_right_big,swim_right_fire;
    ...
    /**true if it is being controlled by accelerometer*/
    private boolean isAccControl=false;
     ...
     */
    private int frameMode=6, tglMovement=-1;
	...
	public Mario() {
		...
		isAccControl=Gdx.input.isPeripheralAvailable(Peripheral.Accelerometer);
		setIsJumping(true);
		dy = STARTING_DY;
		jumpHeight = INITIAL_JUMP_HEIGHT;
		health = STARTING_LIFE;
		
		l_Big = new TextureRegion[] { MarioResourceManager.instance.mario.Mario_Big_Left_Still, MarioResourceManager.instance.mario.Mario_Big_Left_1,
				MarioResourceManager.instance.mario.Mario_Big_Left_2, MarioResourceManager.instance.mario.Mario_Big_Left_Run_1,
				MarioResourceManager.instance.mario.Mario_Big_Left_Run_2, MarioResourceManager.instance.mario.Mario_Big_Crouch_Left,
				MarioResourceManager.instance.mario.Mario_Big_Jump_Left, MarioResourceManager.instance.mario.Mario_Big_Change_Direction_Left };
		r_Big = new TextureRegion[] { MarioResourceManager.instance.mario.Mario_Big_Right_Still, MarioResourceManager.instance.mario.Mario_Big_Right_1,
				MarioResourceManager.instance.mario.Mario_Big_Right_2, MarioResourceManager.instance.mario.Mario_Big_Right_Run_1,
				MarioResourceManager.instance.mario.Mario_Big_Right_Run_2, MarioResourceManager.instance.mario.Mario_Big_Crouch_Right,
				MarioResourceManager.instance.mario.Mario_Big_Jump_Right, MarioResourceManager.instance.mario.Mario_Big_Change_Direction_Right };
		l_Fire = new TextureRegion[]{ MarioResourceManager.instance.mario.Mario_Big_Left_Still_Fire, MarioResourceManager.instance.mario.Mario_Big_Left_1_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Left_2_Fire, MarioResourceManager.instance.mario.Mario_Big_Left_Run_1_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Left_Run_2_Fire, MarioResourceManager.instance.mario.Mario_Big_Crouch_Left_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Jump_Left_Fire, MarioResourceManager.instance.mario.Mario_Big_Change_Direction_Left_Fire };
		r_Fire = new TextureRegion[]{ MarioResourceManager.instance.mario.Mario_Big_Right_Still_Fire, MarioResourceManager.instance.mario.Mario_Big_Right_1_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Right_2_Fire, MarioResourceManager.instance.mario.Mario_Big_Right_Run_1_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Right_Run_2_Fire, MarioResourceManager.instance.mario.Mario_Big_Crouch_Right_Fire,
				MarioResourceManager.instance.mario.Mario_Big_Jump_Right_Fire, MarioResourceManager.instance.mario.Mario_Big_Change_Direction_Right_Fire };
	    
		  swim_left_small=MarioResourceManager.instance.mario.Mario_swim_left_small;
		   swim_right_small=MarioResourceManager.instance.mario.Mario_swim_right_small;
		   swim_left_big=MarioResourceManager.instance.mario.Mario_swim_left_big;
		   swim_right_big=MarioResourceManager.instance.mario.Mario_swim_right_big;
		   swim_left_fire=MarioResourceManager.instance.mario.Mario_swim_left_fire;
		   swim_right_fire=MarioResourceManager.instance.mario.Mario_swim_right_fire;
		...
		l_small = MarioResourceManager.instance.mario.Mario_Small_Left;
		r_small = MarioResourceManager.instance.mario.Mario_Small_Right;
		...
		TextureRegion flipped = MarioResourceManager.instance.mario.Mario_Flip;
	...
	public void update(TileMap map, float time,boolean lockInput) {
		...
		if (isSystemDriven){
			...
			if (getY()>TileMap.tilesToPixels(map.getHeight()))StageClear();
			return;
		}
		...
		if (!isAlive){
			...
			setY( getY() + getdY()*time);
			if (getY()>TileMap.tilesToPixels(map.getHeight()))GameOver();
			return;
		}
		
		if (loadFireBall && FireBall.isReady()){
			loadFireBall=false;
			map.creaturesToAdd().add(new FireBall((int) (left?x-3:x+getWidth()-2),(int)(y+getHeight()*0.14f),left?-2:2));
			if (left){
				setAnimation(fireLeftAnim);
			}else{
				setAnimation(fireRightAnim);
			}
			MarioSoundManager.instance.playFireBall();
		}
		...
		if(!frictionLock && isLeftHeld && isAccControl ){
			float v_max=TERMINAL_WALKING_DX+(Math.abs(Gdx.input.getAccelerometerY())-2.5f)/20;
			if(v_max>TERMINAL_RUNNING_DX)v_max=TERMINAL_RUNNING_DX;
			float dv=v_max/10;
    	...
    	}else if(!frictionLock && isRightHeld && isAccControl){
    		float v_max=TERMINAL_WALKING_DX+(Math.abs(Gdx.input.getAccelerometerY())-2.5f)/20;
    		if(v_max>TERMINAL_RUNNING_DX)v_max=TERMINAL_RUNNING_DX;
    		float dv=v_max/10;
		...
		ArrayList<Vector2> xTile = TileMap.getTileCollisionAll(map, this, getX(), getY(), newXCalc, getY());
		ArrayList<Vector2> yTile = TileMap.getTileCollisionAll(map, this, getX(), getY(), getX(), newYCalc); 
		...
		if(oldX < 0) { // Collision with left side of map.
			setX(TileMap.tilesToPixels(0));
			slowSpeed(20);
		} else if(oldX > TileMap.tilesToPixels(map.getWidth()) - 21) { // Collision with right side of map.
			setX(TileMap.tilesToPixels(map.getWidth()) - 21);
			slowSpeed(20);
		} else {
		...
		if(oldY > TileMap.tilesToPixels(map.getHeight()) - getHeight()) { // Off the bottom of the map.
			...
			if(numOfYTiles == 0) {
				...
				if(c == Collision.NORTH) { // Downward collision with tile.
					...
					setY(TileMap.tilesToPixels(ytp.y + 1));
		...
		if(!creature.isPlatform() && creature.isCollidable() || creature instanceof BoomRang) { 
			...
			if(collision && !(creature instanceof Score)) {
				if(creature instanceof LevelComplete) {
					...
					this.setdX(0);
					this.setdY(0.075f);
					//MarioSoundManager.instance.playCelebrate();
					...
					//MarioSoundManager.instance2.playCelebrate();
					grace=40;
					creature.kill();
					if (((Mushroom)creature).isHealthUp()){
						if (health<3){
							MarioSoundManager.instance.playHealthUp();
							health++;
							Settings.setLives(health);
						}else{
							MarioSoundManager.instance.playBonusPoints();
						} 
					}else{
						if (small){
							small=false;
							...
							toggleMarioFrames(1);
							MarioSoundManager.instance.playItemSprout();
							MarioSoundManager.instance.playPowerUp();
						}else{
							MarioSoundManager.instance.playBonusPoints();
						}
					}
				...
				} else if((creature instanceof RedFish) && isJumping() && getdY() > 0 && creature.getY()>+getY()+getHeight()-3) {
				...
				} else if(creature instanceof JumpingFish && isJumping() && getdY() > 0 && creature.getY()>+getY()+getHeight()-3) {
					((JumpingFish) creature).jumpedOn(); // kill RedFish
					this.creatureHop();
					MarioSoundManager.instance.playKick();
					for (int i = 0; i < 8; i++)
			        {
					...
					if (!((FlyRedKoopa)creature).canFly){
						...
						Math.round(creature.getY()+13), map,true,false));
						Settings.addScore(200);
					}else{
					...
					//MarioSoundManager.instance.playKick();
					map.creaturesToAdd().add(new RedShell(Math.round(creature.getX()), 
							Math.round(creature.getY()+13), map,true,((RedKoopa) creature).isGreen()));
					map.creaturesToAdd().add(new Score(Math.round(creature.getX()), Math.round(creature.getY()+13),3));
					Settings.addScore(100);
	...
	public void getsDamaged() {
		if (grace == 0) {
			if (small) {
				...
				hasFire = false;
				MarioSoundManager.instance.playHurt();
				MarioSoundManager.instance.playHealthDown();
				grace = 120;
			}
		}
	}
	...
	public void creatureHop() {
		isJumping = true;
		MarioSoundManager.instance.playStomp();
		setY(y -5); // fix offset
		if(!isShiftHeld) {
	 ...
	 * @param eventID
	 */
    public void processEvent(int eventID,boolean accelerometerDriven) {
    	this.isAccControl=accelerometerDriven;
        if (!isAlive())return;
        if (eventID == -1) {
    		isLeftHeld = true;
    		isRightHeld=false;
...
*/
    ...
    public void draw(Canvas g, int x, int y, int offsetX, int offsetY) {
		g.drawTextureRegion(currentAnimation().getImage(), x + offsetX, y + offsetY,paint);
	}
    */
    ...
    public void killMario() {
		...
		dy = -.25f;
		dx = 0;
		MarioSoundManager.instance.playDie();
	}
	...
	private void StageClear() {
	   ...
	   public void keyPressed(int key) {
	        ...
	        if(key == Keys.CONTROL_LEFT||key == Keys.CONTROL_LEFT||key == Keys.S) {	
