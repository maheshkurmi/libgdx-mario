...
/**
 * Boss Enemy, Kills mario on collision and can be killed by jumping over it 3 times
 * @author maheshkurmi
 *
 */
public class Bowser extends Creature {
	...
	private static TextureRegion left_1,left_2,left_3,left_4,right_1,right_2,right_3,right_4,left_flipped,right_flipped;
	...
	public Bowser(int x, int y) {
		...
		super(x, y);
		if (!initialized){
			 left_1 = MarioResourceManager.instance.creatures.Bowser_Left[4];
			 left_2 = MarioResourceManager.instance.creatures.Bowser_Left[3];
			 left_3 = MarioResourceManager.instance.creatures.Bowser_Left[2];
			 left_4 = MarioResourceManager.instance.creatures.Bowser_Left[1];
			 left_flipped=MarioResourceManager.instance.creatures.Bowser_Left[0];
			 right_1 = MarioResourceManager.instance.creatures.Bowser_Right[0];
			 right_2 = MarioResourceManager.instance.creatures.Bowser_Right[1];
			 right_3 = MarioResourceManager.instance.creatures.Bowser_Right[2];
			 right_4 = MarioResourceManager.instance.creatures.Bowser_Right[3];
			 right_flipped = MarioResourceManager.instance.creatures.Bowser_Right[4];
			 initialized=true;
		}
	...
	public void updateCreature(TileMap map,int time){
		if(!isFlipped()) {
			int tileX = TileMap.pixelsToTiles(dx>0?x-1:x + getWidth() - 1);
			...
			if (x<map.getPlayer().getX()-60 && dx<0){
				...
				BoomRang boomrang;
				if (dx<0){
					boomrang=new BoomRang((int) x-19,(int)(y+6),-2);
					setAnimation(left_fire);
				}else{
					boomrang=new BoomRang((int) x+getWidth()-7,(int)(y+6),2);
