...
public class Creature extends CollidableObject {
	 ...
	 * of producing sounds from the MarioSoundManager.instance. 
	 * 
	 ...
	 */
	public static Collision tileCollisionX(GameTile tile, CollidableObject s) {
	 ...
	 */
	public static Collision tileCollisionY(GameTile tile, CollidableObject s) {
	...
	public void updateCreature(TileMap map, int time) {
		...
		if(!isFlipped) {
			Vector2 xTile = TileMap.getTileCollision(map, this, x, y, newX, y);
			Vector2 yTile = TileMap.getTileCollision(map, this, x, y, x, newY);
			
			this.update(time);
			
			// Update collisions in the x-direction.
			if (oldX < -offMapOffset
					|| oldX > TileMap.tilesToPixels(map.getWidth())
				...
				if (xTile == null) {
					...
					if (!xTile.equals(yTile)) { // Only manage x-collisions that
						...
						this.xCollide(xTile);
						if (dx > 0) {
							x = TileMap.tilesToPixels((int) xTile.x)
									- this.getWidth() - 1;
						} else if (dx < 0) {
							x = TileMap.tilesToPixels((int) (xTile.x + 1));
						}
						...
						if ((xTile.x * 16 > x && dx > 0)
							...
							this.xCollide(xTile);
							if (dx > 0) {
								x = TileMap.tilesToPixels(xTile.x)
										- this.getWidth() - 1;
							} else if (dx < 0) {
								x = TileMap.tilesToPixels(xTile.x + 1);
							}
			...
			if(oldY > TileMap.tilesToPixels(map.getHeight()) + offMapOffset) { // offscreen
				kill();
			} else {
				if(yTile == null) {
					y = newY;
					...
					if(dy > 0) {
						...
						y = TileMap.tilesToPixels(yTile.y) - this.getHeight();
						if (this instanceof FireBall || this instanceof BoomRang){
							this.dy=-0.7f*dy;
							if (dy<0.1)xCollide(new Vector2(0,0)) ;
						...
						if (yTile !=xTile)y = TileMap.tilesToPixels(yTile.y + 1); //if condition added on 7 sept 2013
	...
	private String getBonusInfo(int creatureHitCount){
		if(!(this instanceof RedShell))return "";
